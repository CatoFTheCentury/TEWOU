...
# Objects (Composite)
  - `Composite` (common to each `Composite` object)
    - Methods:
      - `getclientbounds` : returns "client bounds". The composite position in screen coordinates relative to the canvas origin + its width and height; with scaling taken into account. Returns a Bounds type = {x:number,y:number,w:number,h:number}.
      It itself uses multiple functions: `getclientleft`, `getclienttop`, `getclientwidth`, `getclientheight`.
    - Fields:

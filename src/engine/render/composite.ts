...
export namespace Composite {
  ...
  export class Snap extends Composite {
    ...
    public compose():boolean{
      if(this.rprops.delete) {
        if(this.ready) Composite.gl.deleteTexture(this.texture);
  ...
  export class Animation extends Composite {
    ...
    constructor(frames : Array<Snap>, timings: number[] = [200]){
      super();
      this.dynamic = true;
  ...
  export class Frame extends Composite {
    ...
    private crop(){
      if(this.picture.ready){
        ...
        Composite.gl.deleteTexture(Assets.getTexture('croptex'));
        this.picture.ready = false;
      }
      if(!this.picture.ready){
        Assets.addTexture('croptex',this.texture,{w:100,h:100})
        let img = new Image('croptex',
